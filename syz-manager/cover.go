...
func coverToPCs(target *targets.Target, modules []host.KernelModule,
	...
	return offsetsToPCs(target, modules, rg, offsets)
}

func offsetsToPCs(target *targets.Target, modules []host.KernelModule, rg *cover.ReportGenerator,
	offsets map[string][]uint32) []uint64 {
	var pcs []uint64
	for mod, offs := range offsets {
